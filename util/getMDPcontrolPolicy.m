function [policy_all_partitions] = getMDPcontrolPolicy(evalCacheParams)
...
for partition_idx = 1:deglifePartitions_num
    ...
    if(fileExists)
        ...
        valueFuncation_kp1 = zeros(z_num,x_num,h_num,priorBeliefSpacePartition_num_Np1);
        ...
        for horizon_idx = timeHorizonsPerDay:-1:1            
            for k_in_horizon = k_num_in_horizon:-1:1
                ...
                controllerDecisions_k = zeros(z_num,x_num,h_num,beliefSpacePartition_num_k);
                valueFunction_k = zeros(z_num,x_num,h_num,beliefSpacePartition_num_k);
                opt_start = tic;
                                            ...
                                            if(k_in_day<k_num_in_day)
                                                for x_kp1_idx = 1:x_num
                                                    for h_kp1_idx = 1:h_num
                                                        temp = temp + valueFuncation_kp1(z_kp1_idx_t,x_kp1_idx,h_kp1_idx,p_kp1_idx)*...
                ...
                if(h_num==2) % only for 2D case. Combining in higher dimensions can lead to nonconvex paritions.
                    if(beliefSpacePartition_num_k>1)
                        rows_count= 2*x_num*h_num*z_num;
                            ...
                            for z_k_idx = 1:z_num
                                for x_k_idx = 1:x_num
                                    for h_k_idx = 1:h_num
                                        ...
                                        offset = offset + 2;
                                        p_sim_check_matrix(p_idx,range) = [controllerDecisions_k(z_k_idx,x_k_idx,h_k_idx,p_idx);valueFunction_k(z_k_idx,x_k_idx,h_k_idx,p_idx)];
                                    end
                                end
                            end
                        end
                        ...
                        if(length(temp)~=beliefSpacePartition_num_k)
                            ...
                            controllerDecisions_with_similarity_k = zeros(z_num,x_num,h_num,beliefSpacePartition_with_similarity_num_k);
                            ...
                            valueFuncation_with_similarity_k = zeros(z_num,x_num,h_num,beliefSpacePartition_with_similarity_num_k);
            ...
            if(horizon_idx>1)
                ...
                valueFuncation_Np1 = zeros(z_num,x_num,h_num,priorBeliefSpacePartition_num_Np1);
