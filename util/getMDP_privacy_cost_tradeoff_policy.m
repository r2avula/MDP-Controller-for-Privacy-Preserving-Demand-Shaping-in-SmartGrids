function [policy_all_partitions] = getMDP_privacy_cost_tradeoff_policy(evalCacheParams)
...
for partition_idx = 1:deglifePartitions_num
    ...
    if(fileExists)
        ...
        valueFuncation_kp1 = zeros(z_num,controller_x_num,h_num,priorBeliefSpacePartition_num_Np1);
        ...
        for horizon_idx = timeHorizonsPerDay:-1:1            
            for k_in_horizon = k_num_in_horizon:-1:1
                ...
                controllerDecisions_k = zeros(z_num,controller_x_num,h_num,beliefSpacePartition_num_k);
                valueFunction_k = zeros(z_num,controller_x_num,h_num,beliefSpacePartition_num_k);
                opt_start = tic;
                            ...
                            for z_k_idx = 1:z_num
                                ...
                                for d_k_idx = 1:d_num
                                    ...
                                    if(isnan(L_bar)||isnan(z_kp1_idx_t)||z_kp1_idx_t<=0)
                                        ...
                                        y_k_idx = min((x_k_idx+x_offset) + (d_k_idx+d_offset) - y_offset,adversary_x_num);
                                        if(y_k_idx<1)
                                            alphaVector_k(d_k_idx) = inf;
                                        else
                ...
                if(h_num==2) % only for 2D case. Combining in higher dimensions can lead to nonconvex paritions.
                    if(beliefSpacePartition_num_k>1)
                        rows_count= 2*controller_x_num*h_num*z_num;
                        ...
                        if(length(temp)~=beliefSpacePartition_num_k)
                            ...
                            controllerDecisions_with_similarity_k = zeros(z_num,controller_x_num,h_num,beliefSpacePartition_with_similarity_num_k);
                            ...
                            valueFuncation_with_similarity_k = zeros(z_num,controller_x_num,h_num,beliefSpacePartition_with_similarity_num_k);
            ...
            if(horizon_idx>1)
                ...
                valueFuncation_Np1 = zeros(z_num,controller_x_num,h_num,priorBeliefSpacePartition_num_Np1);
