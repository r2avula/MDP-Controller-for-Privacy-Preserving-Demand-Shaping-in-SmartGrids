function [policy_all_partitions] = getMDP_deviation_privacy_cost_tradeoff(evalCacheParams)
...
policy_all_partitions = cell(deglifePartitions_num,num_tradeoffs,num_tradeoffs);
...
for partition_idx = 1:deglifePartitions_num
    for tradeOff_omega_withinUtility_idx = 2:num_tradeoffs-1
        for tradeOff_sigma_forcost_idx = 2:num_tradeoffs
            ...
            policyParams.costWeight = round((1 - tradeOff_sigma_forcost(tradeOff_sigma_forcost_idx))*evalCacheParams.costWeight,paramsPrecisionDigits);
            ...
            if(fileExists)
                ...
                costWeight = policyParams.costWeight;
                ...
                valueFuncation_kp1 = zeros(z_num,x_num,h_num,y_num,priorBeliefSpacePartition_num_Np1);
                ...
                for horizon_idx = timeHorizonsPerDay:-1:1
                    for k_in_horizon = k_num_in_horizon:-1:1
                        ...
                        J_kp1_inv_vertices_num = J_kp1_vertices_num*(x_num+1);
                        ...
                        if(abs(det(adversary_P_HgHn1(:,:,k_in_day)))>=paramsPrecision)
                            ...
                            J_kp1_inv_vertices(:,(x_num)*J_kp1_vertices_num+1:(x_num+1)*J_kp1_vertices_num) = temp;
                        end
                        ...
                        fs_filter_vertices_first_elem = zeros(x_num,1);
                        for y_k_idx = 1:x_num
                        ...
                        partitionTransitionMap_k = zeros(x_num,beliefSpacePartition_num_k);
                        optAdvStratVectorIdx_k = zeros(x_num,beliefSpacePartition_num_k);
                        ...
                        controllerDecisions_k = zeros(z_num,x_num,h_num,y_num,beliefSpacePartition_num_k);
                        valueFunction_k = zeros(z_num,x_num,h_num,y_num,beliefSpacePartition_num_k);
                        opt_start = tic;
                                        ...
                                        for z_k_idx = 1:z_num
                                            ...
                                            for d_k_idx = 1:d_num
                                                ...
                                                if(isnan(L_bar)||isnan(z_kp1_idx_t)||z_kp1_idx_t<=0)
                                                    ...
                                                    y_k_idx = min((x_k_idx+x_offset) + (d_k_idx+d_offset) - y_offset,x_num);
                                                    if(y_k_idx<1)
                                                        alphaVector_k(d_k_idx) = inf;
                                                    else
                                                        p_kp1_idx = partitionTransitionMap_k(y_k_idx,p_idx);
                                                        H_guess_k = optAdvStratVectorIdx_k(y_k_idx,p_idx);
                                                        temp = privacyWeight*C_HgHh(h_k_idx,H_guess_k) + deviationWeight*abs((y_kn1_idx - y_k_idx)*p_pu) + costWeight*L_bar;
                                            ...
                                            controllerDecisions_k(z_k_idx,x_k_idx,h_k_idx,p_idx) = d_k_idx_star;
                                            valueFunction_k(z_k_idx,x_k_idx,h_k_idx,p_idx) = opt_val;
                                        end
                                    end
                                end
                            end
                        ...
                        if(h_num==2) % only for 2D case. Combining in higher dimensions can lead to nonconvex paritions.
                            if(beliefSpacePartition_num_k>1)
                                rows_count= 2*x_num*h_num*z_num*y_num;
                                ...
                                if(length(temp)~=beliefSpacePartition_num_k)
                                    ...
                                    controllerDecisions_with_similarity_k = zeros(z_num,x_num,h_num,y_num,beliefSpacePartition_with_similarity_num_k);
                                    ...
                                    valueFuncation_with_similarity_k = zeros(z_num,x_num,h_num,y_num,beliefSpacePartition_with_similarity_num_k);
                    ...
                    if(horizon_idx>1)
                        ...
                        valueFuncation_Np1 = zeros(z_num,x_num,h_num,y_num,priorBeliefSpacePartition_num_Np1);
            ...
            policy_all_partitions{partition_idx,tradeOff_omega_withinUtility_idx,tradeOff_sigma_forcost_idx} = policy;
        end
    end    
end
...
for tradeOff_sigma_forcost_idx = 2:num_tradeoffs-1
    evalCacheParams_t = evalCacheParams;
    evalCacheParams_t.privacyWeight = 0;
    evalCacheParams_t.tradeOff_omega_withinUtility_idx = tradeOff_omega_withinUtility_idx;
    evalCacheParams_t.tradeOff_sigma_forcost_idx = tradeOff_sigma_forcost_idx;
    policy_all_partitions{:,tradeOff_omega_withinUtility_idx,tradeOff_sigma_forcost_idx} = getMDP_deviation_cost_tradeoff(evalCacheParams_t);
end
...
for tradeOff_sigma_forcost_idx = 2:num_tradeoffs-1
    evalCacheParams_t = evalCacheParams;
    evalCacheParams_t.deviationWeight = 0;
    evalCacheParams_t.tradeOff_omega_withinUtility_idx = tradeOff_omega_withinUtility_idx;
    evalCacheParams_t.tradeOff_sigma_forcost_idx = tradeOff_sigma_forcost_idx;
    policy_all_partitions{:,tradeOff_omega_withinUtility_idx,tradeOff_sigma_forcost_idx} = getMDP_privacy_cost_tradeoff(evalCacheParams_t);
end
end
